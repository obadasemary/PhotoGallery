	       ...
	       if(count($_FILES['photo']) > 0)
           {
                if(is_uploaded_file($_FILES['photo']['tmp_name']))
                {
                    if(move_uploaded_file($_FILES['photo']['tmp_name'], "user_photos/". $_FILES['photo']['name']))
                    {
                        ...
                        $database->query("INSERT INTO user_photo VALUES (NULL, {$_SESSION['user_id']}, 'profile', '$photo', NOW())");
                        if($database->affected_rows() == 1)
                        {
                            $database->query("UPDATE user SET user_picture = '$photo' WHERE user_id = {$_SESSION['user_id']}");
...
<?php include_layout_template('admin_header.php'); ?>
  ...
  <form action="profile_photo.php" enctype="multipart/form-data" method="POST">
